...
<template>
  <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" @click.self="$emit('close')">
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md p-6" role="dialog" aria-modal="true">
      <h2 class="text-lg font-bold mb-4 text-gray-900 dark:text-gray-100">{{ task ? '编辑任务' : '新建任务' }}</h2>

      <div class="mb-4">
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">标题 <span class="text-red-500">*</span></label>
        <input ref="titleInput" v-model="title" type="text" class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 focus:border-transparent" placeholder="输入任务标题" />
      ...
      <div class="flex justify-end gap-3">
        ...
        <button @click="handleSubmit" class="px-4 py-2 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 transition-colors">{{ task ? '保存' : '创建' }}</button>
      </div>
    </div>
  </div>
</template>
